...
int main()
{
  ...
  tudat_learn::Dataset new_dataset(std::vector<type_test>({vec1, vec2}), std::vector<type_test>({vec2, vec1}));
  auto new_ptr = std::make_shared< tudat_learn::Dataset<type_test, type_test> >(new_dataset);

  tudat_learn::RBFN<type_test, type_test> rbfn_instance(new_ptr, std::make_shared<tudat_learn::CubicRBF<float>>(tudat_learn::CubicRBF<float>()));
