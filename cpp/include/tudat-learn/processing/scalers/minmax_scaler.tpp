...
void MinMaxScaler<Datum_t, Label_t>::fit(const Dataset<Datum_t, Label_t> &dataset) {
  ...
  for(int i = 1; i < dataset.size(); ++i) {
    min_in_dataset = this->min_elementwise(min_in_dataset, dataset.data_at(i));
    max_in_dataset = this->max_elementwise(max_in_dataset, dataset.data_at(i));
  }
...
void MinMaxScaler<Datum_t, Label_t>::fit(const Dataset<Datum_t, Label_t> &dataset, const std::vector<int> &fit_indices) {
  ...
  for(int i = 1; i < fit_indices.size(); ++i) {
    min_in_dataset = this->min_elementwise(min_in_dataset, dataset.data_at(fit_indices.at(i)));
    max_in_dataset = this->max_elementwise(max_in_dataset, dataset.data_at(fit_indices.at(i)));
  }
...
Dataset<Datum_t, Label_t> MinMaxScaler<Datum_t, Label_t>::transform(Dataset<Datum_t, Label_t> dataset) const {
  for(int i = 0; i < dataset.size(); ++i) {
    ...
    Datum_t scaled_datum = dataset.data_at(i) - min_in_dataset;
            scaled_datum = this->operator_divide_elementwise(scaled_datum, difference_dataset) * difference_range;
            scaled_datum = this->operator_add_scalar_elementwise(scaled_datum, range.first);
...
Datum_t MinMaxScaler<Datum_t, Label_t>::inverse_transform(Datum_t datum) const {
  ...
  datum = this->operator_subtract_scalar_elementwise(datum, range.first) / difference_range;
  datum = this->operator_multiply_elementwise(datum, difference_dataset) + min_in_dataset;
